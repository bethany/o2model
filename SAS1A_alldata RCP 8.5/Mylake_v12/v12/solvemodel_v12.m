...
% ===Switches===
snow_compaction_switch=0;       %snow compaction: 0=no, 1=yes
...
N_sed=12; %total number of layers in the sediment column
...
K_sed=0.15;      %thermal diffusivity of the sediments (m2 day-1) 
...
cp_sed=3000;       %specific heat capasity of the sediments (J kg-1 K-1)
%=======
...
for i = 1:length(tt)
    ...
    Pz_prev=Pz; PPz_prev=PPz; DOPz_prev=DOPz; Chlz_prev=Chlz; Oxygenz_prev = Oxygenz;%for monitoring purposes
...
load '~/Desktop/Making_a_model/SAS1A_alldata/SAS1ASal_alldata.mat';
Conddt = SAS1ASal';
pressure = [1 2 3]';

   Tprof_prev=Tz; %temperature profile at previous time step (for modified convection.m)
   rho = gsw_rho(Conddt(:,i),max(0,Tz(:)),pressure);  % Density (kg/m3)                                                      
...
[Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,Oxygenz] = convection_v12_1a(Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,Tprof_prev,Vz,Cw,f_par,lambdaz_wtot_avg,zz,swa_b0,tracer_switch,1,Conddt(:,i),Oxygenz);
...
[Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,Oxygenz] = convection_v12_1a(Tz,Cz,Sz,Pz,Chlz,PPz,DOPz,DOCz,Tprof_prev,Vz,Cw,f_par,lambdaz_wtot_avg,zz,swa_b0,tracer_switch,1,Conddt(:,i),Oxygenz);
   ...
   rho = gsw_rho(Conddt(:,i),max(0,Tz(:)),pressure);  % Water density (kg m-3)                                                      
...
% load '~/Desktop/Making_a_model/SAS1A_alldata/KWGroundTemp_alldata.mat';


% if(IceIndicator==0) % Bethany's cooling from sediment !!!!!
%   Tz(1) = (Tz(1) + KWGroundTemp(i,1))/1.4;
%   Tz(2) = (Tz(2) + KWGroundTemp(i,1))/1.0;
%   Tz(3) = (Tz(3) + KWGroundTemp(i,1))/1.2;
% end


% if(IceIndicator==1) % Bethany's cooling from sediment !!!!!
%   Tz(2) = (Tz(2) + KWGroundTemp(i,1))/5; 
% end
          ...
          if (isempty(inx)==0); %if water column not already fully mixed
           ...
           KP_ratio=TKE/POE;
            if (KP_ratio>=1)
             ...
             rho = gsw_rho(Conddt(:,i),max(0,Tz(:)),pressure);
             TKE=TKE-POE;
...
% Bethany - Oxygen Model taken from Stefan & Fang, 1994
...
%% ************************************** %%
      ...
      % 2.2 Oxygen Production by photosynthesis

        % 2.2.3 Chl-a concentration
          % This is given by var Chlz

        % 2.2.2 Light limitation
          
          % K1, Einst M-2 h-1
          LightLimCoeff = 0.687 * 1.086^(Tz(1)-20); % K1, Einst M-2 h-1
          ...
          % I: Photosynthesitically Active Radiation, Einst M-2 h-1
            Irradiance = U_sw_z(1);

          LightLimit = (Irradiance*(1+2*sqrt(LightLimCoeff/LightInhibCoeff)))/(Irradiance + LightLimCoeff + (Irradiance^2/LightInhibCoeff));
            % I: Photosynthesitically Active Radiation, Einst M-2 h-1
            % K1,K2: light limitation and inhibitation coefficients, Einst M-2 h-1

        % 2.2.1 Rate of oxygen production
          Photosynthesis_MaxRate = 9.4 * 1.036^(Tz(1)-20); % units: [mg O2 (mg Chla)-1 h-1]

          Photosynthesis = Photosynthesis_MaxRate * Attn_z(1) * Chlz(1);


      % 2.3 Surface reaeration

        % Ke: surface oxygen transfer velocity (m day-1)
          % U_10 = Wt(i,6); % wind speed (m/s); might have to review and calculate true U_10 later.
          
          U_10 = Wt(i,6)*(10/2)^0.143; % Wind profile power law
          e = 2.71828;

          Ke = (0.02256 * (0.10656 * e^(-0.0627*Tz(1)) + 0.00495)^(-0.5) * U_10^(1.64));

        % Zs: thickness of the surface layer (m)
          Ed = 1.18 - 0.533*0.001*DOCz(1) + 0.143*0.05 - 0.117*1.5 - 0.073*Chlz(1);

        % C_sat: saturated oxygen concentration in the first layer (mg L-1) 
          C_sat = 14.652 - (0.41022 * Tz(1)) + (0.007991 * (Tz(1)^2)) - (0.000077774 * (Tz(1)^3));


        % FS: Oxygen transfer rate, Holley 1977
        FS = (Ke/Ed)*(C_sat - Oxygenz_prev(1));
        % FS = (Ke/Zs)*(C_sat - C_s);

      % Respiration
      
        Kr = 0.05; % respiration rate coefficient; d-1; ranges from 0.02 to 0.60
        RespTheta = 1.045; % temperature adjustment coefficient; d-1

      Respiration = (1/0.008) * Kr * RespTheta^(Tz(1)-20) * Chlz(1);

      % Biochemical oxygen demand

        Kb = 2; % first-order decay coefficient; d-1; ranges from 0.02 to 3.4
        BOD = 2; % detritus as oxygen equivalent; mg L-1; represents eutrophic lake.

        if (Tz(1) < 20)
          BODTheta = 1.13; % temperature-adjustment coefficient
        else
          BODTheta = 1.047;
        end

        SBOD = Kb * BODTheta^(Tz(1)-20) * BOD;
      ...
      % Sedimentary oxygen demand  
          ...
          if (Tz(1) < 10)
            Theta_s = 1.13; % temperature-adjustment coefficient
          else
            Theta_s = 1.065;
          end

          Sb = Sb_20 * Theta_s^(Tz(1) - 20); 
          SedArea = 2040; % m^2


        SSOD = (Sb / SedArea) * (Az(1,1)/1);

        

    if (IceIndicator==1) % In winter
      % Surface
        Oxygenz(1) = Cz(1) + Photosynthesis - Respiration - SSOD - SBOD;
          O2Factors_Surface(1,i) = Photosynthesis;
          O2Factors_Surface(2,i) = Respiration;
          O2Factors_Surface(3,i) = SSOD;
          O2Factors_Surface(4,i) = SBOD;
          O2Factors_Surface(5,i) = 0;

    else % In ice-free periods
      % Surface
        Oxygenz(1) = Cz(1) + Photosynthesis - Respiration - SSOD - SBOD + FS;
          O2Factors_Surface(1,i) = Photosynthesis;
          O2Factors_Surface(2,i) = Respiration;
          O2Factors_Surface(3,i) = SSOD;
          O2Factors_Surface(4,i) = SBOD;
          O2Factors_Surface(5,i) = FS;
    end
...
%% ************************************** %%
      ...
      % 2.2 Oxygen Production by photosynthesis
        ...
        % 2.2.2 Light limitation
          ...
          % K1, Einst M-2 h-1
          LightLimCoeff = 0.687 * 1.086^(Tz(2)-20); % K1, Einst M-2 h-1
        ...
        % 2.2.1 Rate of oxygen production
          Photosynthesis_MaxRate = 8 * 1.036^(Tz(2)-20); % units: [mg O2 (mg Chla)-1 h-1]

          Photosynthesis = Photosynthesis_MaxRate * Attn_z(2) * Chlz(2);


      % 2.3 Surface reaeration

        % not necessary for bottom waters.

      % Respiration
      
        Kr = 0.02; % respiration rate coefficient; d-1; ranges from 0.02 to 0.60
        RespTheta = 1.045; % temperature adjustment coefficient; d-1

      Respiration = (1/0.008) * Kr * RespTheta^(Tz(2)-20) * Chlz(2);

      % Biochemical oxygen demand

        Kb = 1; % first-order decay coefficient; d-1; ranges from 0.02 to 3.4
        BOD = 1; % detritus as oxygen equivalent; mg L-1; represents eutrophic lake.
      ...
      % Sedimentary oxygen demand  
          ...
          Sb = Sb_20 * Theta_s^(Tz(2) - 20); 
          SedArea = 710; % m^2


        SSOD = (Sb / SedArea) * (Az(2,1)/1);


    if (IceIndicator==1) % In winter
      % Bottom
        Oxygenz(2) = Cz(2) + Photosynthesis - Respiration - SSOD - SBOD;
          O2Factors_Middle(1,i) = Photosynthesis;
          O2Factors_Middle(2,i) = Respiration;
          O2Factors_Middle(3,i) = SSOD;
          O2Factors_Middle(4,i) = SBOD;
          O2Factors_Middle(5,i) = 0;
    else % In ice-free periods
      % Bottom
        Oxygenz(2) = Cz(2) + Photosynthesis - Respiration - SSOD - SBOD;
          O2Factors_Middle(1,i) = Photosynthesis;
...
%% ************************************** %%
      ...
      % 2.2 Oxygen Production by photosynthesis
        ...
        % 2.2.2 Light limitation
          ...
          % K1, Einst M-2 h-1
          LightLimCoeff = 0.687 * 1.086^(Tz(3)-20); % K1, Einst M-2 h-1
        ...
        % 2.2.1 Rate of oxygen production
          Photosynthesis_MaxRate = 8 * 1.036^(Tz(3)-20); % units: [mg O2 (mg Chla)-1 h-1]

          Photosynthesis = Photosynthesis_MaxRate * Attn_z(3) * Chlz(3);
      ...
      Respiration = (1/0.008) * Kr * RespTheta^(Tz(3)-20) * Chlz(3);
      ...
      % Sedimentary oxygen demand  
          ...
          Sb = Sb_20 * Theta_s^(Tz(3) - 20); 
          SedArea = 240; % m^2


        SSOD = (Sb / SedArea) * (Az(3,1)/1);


    if (IceIndicator==1) % In winter
      % Bottom
        Oxygenz(3) = Cz(3) + Photosynthesis - Respiration - SSOD - SBOD;
          O2Factors_Bottom(1,i) = Photosynthesis;
          O2Factors_Bottom(2,i) = Respiration;
          O2Factors_Bottom(3,i) = SSOD;
          O2Factors_Bottom(4,i) = SBOD;
          O2Factors_Bottom(5,i) = 0;
    else % In ice-free periods
      % Bottom
        Oxygenz(3) = Cz(3) + Photosynthesis - Respiration - SSOD - SBOD;
          O2Factors_Bottom(1,i) = Photosynthesis;
