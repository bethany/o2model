...
if(IceIndicator==0) % Bethany's cooling from sediment !!!!!
  ...
  Tz(2) = (Tz(2) + KWGroundTemp(i,1))/1.1;
  Tz(3) = (Tz(3) + KWGroundTemp(i,1))/1.82;
end
          ...
          if (isempty(inx)==0); %if water column not already fully mixed
           ...
           KP_ratio=TKE/POE;
            if (KP_ratio>=1)
             ...
             Tz(1:zb+1)= Tmix - (Tmix / (Tmix + 1)); % Bethany
             ...
             Tz(1:zb)=Tmix - (Tmix / (Tmix + 10)); %- (0.3 .* Tmix); % Bethany
...
%% ************************************** %%
      ...
      Respiration = (1/0.008) * Kr_epilimnion * RespTheta_epilimnion^(Tz(1)-20) * Chlz(1);
      ...
      % Biochemical oxygen demand
        ...
        SBOD = Kb_epilimnion * BODTheta^(Tz(1)-20) * BOD_epilimnion;
      ...
      % Sedimentary oxygen demand  
          ...
          Sb = Sb20_epilimnion * Theta_s^(Tz(1) - 20);

        SSOD = (Sb / SedArea_epilimnion) * (Az(1,1)/1);
...
%% ************************************** %%
      ...
      Respiration = (1/0.008) * Kr_metalimnion * RespTheta_metalimnion^(Tz(2)-20) * Chlz(2);
      ...
      % Biochemical oxygen demand
        ...
        SBOD = Kb_metalimnion * BODTheta^(Tz(2)-20) * BOD_metalimnion;

      % Sedimentary oxygen demand  

          Sb20_metalimnion = 1.0;
          ...
          Sb = Sb20_metalimnion * Theta_s^(Tz(2) - 20); 
          SedArea_metalimnion = 3000; % m^2


        SSOD = (Sb / SedArea_metalimnion) * (Az(1,1)/1);
...
%% ************************************** %%
      ...
      % Respiration
        ...
        Kr_hypolimnion = 0.02; % respiration rate coefficient; d-1; ranges from 0.02 to 0.60
        RespTheta_hypolimnion = 1.045; % temperature adjustment coefficient; d-1

      Respiration = (1/0.008) * Kr_hypolimnion * RespTheta_hypolimnion^(Tz(3)-20) * Chlz(3);

      % Biochemical oxygen demand

        Kb_hypolimnion = 1; % first-order decay coefficient; d-1; ranges from 0.02 to 3.4
        BOD_hypolimnion = 1; % detritus as oxygen equivalent; mg L-1; represents eutrophic lake.
        ...
        SBOD = Kb_hypolimnion * BODTheta^(Tz(3)-20) * BOD_hypolimnion;

      % Sedimentary oxygen demand  

          Sb20_hypolimnion = 1.0;
          ...
          Sb = Sb20_hypolimnion * Theta_s^(Tz(3) - 20); 
          SedArea_hypolimnion = 3000; % m^2


        SSOD = (Sb / SedArea_hypolimnion) * (Az(1,1)/1);
